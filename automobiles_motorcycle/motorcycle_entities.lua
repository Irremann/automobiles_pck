        ...
        if is_attached == false then
		    ...
		    if itmstck then
		   		local inv = puncher:get_inventory()
					if not self.driver and self.owner == name and not (creative and creative.is_enabled_for and creative.is_enabled_for(name)) then
						local leftover = inv:add_item("main", "automobiles_motorcycle:motorcycle")
						if not leftover:is_empty() then	minetest.add_item(self.object:get_pos(), leftover)
						end
